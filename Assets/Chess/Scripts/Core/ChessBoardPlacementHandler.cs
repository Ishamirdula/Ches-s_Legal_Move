...
public sealed class ChessBoardPlacementHandler : MonoBehaviour
{
    ...
    [SerializeField] private GameObject _redHighlightPrefab;        //Red highlight prefab for showing attackable enemy tiles
    ...
    internal void HighlightRed(int row, int col)               //Red Highlights a tile
    {
        var tile = GetTile(row, col)?.transform;
        if (tile == null)
        {
        ...
        Instantiate(_redHighlightPrefab, tile.position, Quaternion.identity, tile);
    }
    ...
    internal void ClearHighlights()
    {
        for (var i = 0; i < 8; i++)
        {
            for (var j = 0; j < 8; j++)
            {
                ...
                // Destroy only highlighter objects (optional: use tag or name)
